...
<ul class="sidebar-nav" id="sidebar-nav">
  ...
  <li class="nav-item">
          ...
          <ul id="charts-nav" class="nav-content collapse show" data-bs-parent="#sidebar-nav">
            <li>
              <a href="{{route('report.data1')}}" class="active">
                ...
                <tbody>
                    @foreach($offices as $office)
                        <tr>
                            ...
                            <td class="text-center">{{ $office->strongly_agree }}</td>
                            <td class="text-center">{{ $office->agree }}</td>
                            <td class="text-center">{{ $office->neutral }}</td>
                            <td class="text-center">{{ $office->disagree }}</td>
                            <td class="text-center">{{ $office->strongly_disagree }}</td>
                            <td class="text-center">{{ $office->na }}</td>
                            <td class="text-center">{{ $office->total_responses }}</td>
                            <td class="text-center">{{ number_format($office->overall_rating_percentage, 2) }}%</td>
                        </tr>
                    @endforeach
                    <tr>
                    <td><strong>Overall</strong></td>
                          <td class="text-center">{{ $overall->total_strongly_agree }}</td>
                          <td class="text-center">{{ $overall->total_agree }}</td>
                          <td class="text-center">{{ $overall->total_neutral }}</td>
                          <td class="text-center">{{ $overall->total_disagree }}</td>
                          <td class="text-center">{{ $overall->total_strongly_disagree }}</td>
                          <td class="text-center">{{ $overall->total_na }}</td>
                          <td class="text-center">{{ $overall->total_responses }}</td>
                          <td class="text-center">{{ number_format($overall->overall_rating_percentage, 2) }}%</td>
...
<div class="container">
    ...
    <div class="col-lg-14">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title">SQD 1-8 {{ $office->office_name }} 
                <div class="buttons">
              <button class="copy-btn" id="copyBtn" onclick="copyTable('sqdTable-{{ $office->office_id}}')">
                <i class="bi bi-clipboard"></i> COPY
              </button>
              <button class="download-btn" id="downloadBtn" onclick="downloadCSV('sqdTable-{{ $office->office_id }}')">
                <i class="bi bi-file-earmark-arrow-down"></i> DOWNLOAD CSV
              </button>
            </div>
                </h5>
                <!-- Small tables -->
                <table class="table table-sm" id="sqdTable-{{ $office->office_id }}">
                    <thead>
                        <tr>
                            <th scope="col"></th>
                            <th scope="col">Strongly Agree</th>
                    ...
                    <tbody>
                          ...
                          @foreach($categories as $category)
                              <tr>
                                  ...
                                  <td class="text-center">{{ $office->{$category['prefix'] . '_strongly_agree'} }}</td>
                                  <td class="text-center">{{ $office->{$category['prefix'] . '_agree'} }}</td>
                                  <td class="text-center">{{ $office->{$category['prefix'] . '_neutral'} }}</td>
                                  <td class="text-center">{{ $office->{$category['prefix'] . '_disagree'} }}</td>
                                  <td class="text-center">{{ $office->{$category['prefix'] . '_strongly_disagree'} }}</td>
                                  <td class="text-center">{{ $office->{$category['prefix'] . '_na'} }}</td>
                                  <td class="text-center">{{ $office->{$category['prefix'] . '_total_responses'} }}</td>
                                  <td class="text-center">{{ number_format($office->{$category['prefix'] . '_overall_rating_percentage'}, 2) }}%</td>
                              </tr>
                          ...
                          <tr>
                              <td><strong>Overall</strong></td>
                              <td class="text-center">{{ $overall_totals['strongly_agree'] }}</td>
                              <td class="text-center">{{ $overall_totals['agree'] }}</td>
                              <td class="text-center">{{ $overall_totals['neutral'] }}</td>
                              <td class="text-center">{{ $overall_totals['disagree'] }}</td>
                              <td class="text-center">{{ $overall_totals['strongly_disagree'] }}</td>
                              <td class="text-center">{{ $overall_totals['na'] }}</td>
                              <td class="text-center">{{ $overall_totals['total_responses'] }}</td>
                              <td class="text-center"><strong>{{ number_format($overall_totals['overall_percentage'] / count($categories), 2) }}%</strong></td>
                          </tr>
                      </tbody>
                </table>
            </div>
        ...
        <div class="card-body">
            ...
            <table class="table table-sm" id="sqdTable2">
                ...
                <tbody>
                    ...
                    @foreach($categories as $display_name => $category)
                        @foreach($offices as $office)
                            @php
                        ...
                        <tr>
                            <td>{{ $display_name }}</td>
                            <td class="text-center">{{ $summary_totals['strongly_agree'] }}</td>
                            <td class="text-center">{{ $summary_totals['agree'] }}</td>
                            <td class="text-center">{{ $summary_totals['neutral'] }}</td>
                            <td class="text-center">{{ $summary_totals['disagree'] }}</td>
                            <td class="text-center">{{ $summary_totals['strongly_disagree'] }}</td>
                            <td class="text-center">{{ $summary_totals['na'] }}</td>
                            <td class="text-center">{{ $summary_totals['total_responses'] }}</td>
                            <td class="text-center">
                          
                                  {{ $summary_totals['total_responses'] > 0 ? number_format(($summary_totals['strongly_agree'] + $summary_totals['agree']) / $summary_totals['total_responses'] * 100, 2) : 0 }}%
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <strong>
                                {{ $overall_totals['total_responses'] > 0 ? number_format(($overall_totals['strongly_agree'] + $overall_totals['agree']) / $overall_totals['total_responses'] * 100, 2) : 0 }}%
                            </strong>
                        </td>
                    </tr>
                </tbody>
